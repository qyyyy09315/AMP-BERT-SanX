...
csv_file = r"D:\PyProject\25卓越杯大数据\Data\train_re.csv"
output_csv_with_vectors = r"D:\PyProject\25卓越杯大数据\data2\train_re_with_vectors.csv"
...
# 复制原始数据但删除序列列
df_without_sequence = df.drop(columns=[sequence_column]).reset_index(drop=True)
...
print(f"删除序列列后剩余列数量: {len(df_without_sequence.columns)}")
...
# 保存结果
df_combined.to_csv(output_csv_with_vectors, index=False)
print(f"\n结果已保存到: {output_csv_with_vectors}")
print("Done!")