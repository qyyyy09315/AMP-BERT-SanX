...
class UltimateExtraTreesOptimizer:
    ...
    def comprehensive_model_evaluation(self, y_true, y_pred, model_name="模型"):
        """全面模型评估"""
        print(f"\n{'=' * 60}")
        print(f"📊 {model_name} 全面评估结果")
        print(f"{'=' * 60}")
        ...
        print(f"  MedAE (中位数绝对误差): {medae:.6f}")
        print(f"  最大误差: {max_err:.6f}")
        print(f"  解释方差得分: {explained_variance:.6f}")
        ...
        print(f"\n🎯 相对误差:")
        print(f"  RMSE/目标标准差: {rmse / y_std:.4f}")
        print(f"  MAE/目标标准差: {mae / y_std:.4f}")
    ...
    def ultimate_pipeline_v2_fixed(self, X_train, y_train, X_test, y_test):
        ...
        # 🔥 新增：对最佳模型进行全面的评估
        print(f"\n🔥 对最佳模型 '{best_name}' 进行全面的评估...")
        best_evaluation = self.comprehensive_model_evaluation(y_test, best_pred, f"最佳模型 ({best_name})")

        # 对所有方法进行评估对比
        print(f"\n📋 所有方法评估对比:")
        all_evaluations = {}
        for method_name, (method_pred, method_r2) in methods.items():
            evaluation = self.comprehensive_model_evaluation(y_test, method_pred, f"{method_name}方法")
            all_evaluations[method_name] = evaluation

        return best_pred, best_name, best_r2, methods, best_evaluation, all_evaluations
...
def main():
    ...
    # 执行修复的终极管道
    final_pred, best_method, final_r2, all_methods, best_evaluation, all_evaluations = optimizer.ultimate_pipeline_v2_fixed(
    ...
    evaluation_df = pd.DataFrame({
        '指标': ['R²', 'MSE', 'RMSE', 'MAE', 'MAPE(%)', 'MedAE', '解释方差', '最大误差', '残差均值', '残差标准差'],
        '数值': [
            best_evaluation['r2'],
            best_evaluation['mse'],
            best_evaluation['rmse'],
            best_evaluation['mae'],
            best_evaluation['mape'],
            best_evaluation['medae'],
            best_evaluation['explained_variance'],
            best_evaluation['max_error'],
            best_evaluation['residual_mean'],
            best_evaluation['residual_std']
        ]
    })

    evaluation_path = r"D:\PyProject\25卓越杯大数据\data2\detailed_evaluation_results.csv"
    evaluation_df.to_csv(evaluation_path, index=False, encoding='utf-8-sig')
    print(f"详细评估结果已保存到: {evaluation_path}")
